document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentGame;
    let lights;
    let initialLights; // Зберігає початковий стан блоків
    let timerInterval;
    let seconds = 0;
    ...
    function toggleLights(row, column) {
        ...
        // Toggle the light to the left (if exists)
        if (column > 0) {
            lights[row][column - 1] = lights[row][column - 1] ? 0 : 1;
        }

        // Toggle the light to the right (if exists)
        if (column < lights[row].length - 1) {
            lights[row][column + 1] = lights[row][column + 1] ? 0 : 1;
        }

        // Toggle the light above (if exists)
        if (row > 0) {
            lights[row - 1][column] = lights[row - 1][column] ? 0 : 1;
        }

        // Toggle the light below (if exists)
    ...
    function startGame() {
        fetch('game.json')
            ...
            .then(data => {
                ...
                initialLights = JSON.parse(JSON.stringify(lights)); // Зберігаємо початковий стан
    ...
    function restartGame() {
        ...
        movesCount = 0;
        moves.textContent = movesCount;
        // Відновлюємо гру до початкового стану
    ...
    startButton.addEventListener('click', startGame); // При натисканні на кнопку "Start" починається гра

    restartButton.addEventListener('click', restartGame); // При натисканні на кнопку "Restart" починається нова гра
});
